...
// Custom Symmetric Waveform for the new voice note design
const WaveformBars: React.FC<{ progress: number; isOwn: boolean }> = ({ progress, isOwn }) => {
  // Symmetric heights like the reference image
...
const VoicePlayer: React.FC<VoicePlayerProps> = ({ url, isOwn }) => {
  ...
  // The voice player IS the bubble now, as requested.
  return (
...
const ChatContainer: React.FC<ChatContainerProps> = ({ 
  ...
  const handleSend = () => {
    if (inputText.trim()) {
      ...
      setInputText('');
      // Explicitly focus back to keep keyboard open on mobile
    ...
    <div className="flex flex-col h-full relative overflow-hidden bg-black">
          ...
          {messages.map((msg, idx) => {
            ...
            return (
              <div key={msg.id || idx} className={`flex flex-col ${isOwn ? 'items-end' : 'items-start'} animate-fade-up`}>
                ...
                {isVoice ? (
                  /* Voice Note - Custom bubble as requested */
                  ...
                  <div className={`group relative max-w-[85%] rounded-[1.75rem] px-5 py-4 shadow-lg transition-all ${
                    ...
                    {msg.image_url && <img src={msg.image_url} alt="attached" className="rounded-2xl mt-2 max-h-80 w-full object-cover border border-white/5" />}
      ...
      {isLocked && (
        <div className="absolute inset-0 bg-black/95 backdrop-blur-3xl z-40 flex flex-col items-center justify-center text-center p-12 pointer-events-none animate-in fade-in duration-700">
          ...
          <p className="text-slate-500 text-[10px] font-black mt-4 uppercase tracking-[0.4em] leading-relaxed max-w-[240px]">Deep focus protocol engaged.</p>
        </div>
      )}

      {/* Redesigned Input Area based on reference screenshot */}
      <div className={`p-6 bg-[#050505] border-t border-white/5 transition-all duration-700 shrink-0 ${isLocked ? 'opacity-0 translate-y-10 pointer-events-none' : 'opacity-100 translate-y-0'}`}>
        <div className="max-w-4xl mx-auto flex items-center gap-4">
          <button disabled={isProcessing} onClick={() => fileInputRef.current?.click()} className="p-3 text-slate-500 hover:text-white transition-colors hover:bg-white/5 rounded-xl disabled:opacity-20">
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>
          </button>
          <input type="file" ref={fileInputRef} className="hidden" accept="image/*" onChange={handleImageUpload} />
          
          <div className="flex-1 relative flex items-center bg-[#0d1117] border border-white/5 rounded-[2rem] transition-all focus-within:border-white/10 px-6">
            <textarea 
              ref={textareaRef}
              value={inputText} 
              ...
              rows={1} 
              className="w-full bg-transparent py-4 text-[15px] text-white focus:outline-none resize-none placeholder-slate-600 disabled:opacity-50" 
            />
            {inputText.trim() && (
              <button onClick={handleSend} className="text-white ml-2 p-1 hover:scale-110 transition-transform">
          ...
          <button 
            ...
            className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all active:scale-95 disabled:opacity-20 ${
              isRecording 
                ...
                : 'bg-[#161b22] text-slate-400 hover:bg-[#21262d] hover:text-white'
            }`}
          >
